...
Sound resources have one build-time source of truth. `passport.toml` declares ordered `[[sounds]]` entries; the filesystem-free sound metadata compiler validates names and PCM bytes, assigns declaration-order IDs, derives MoonBit symbols, and emits the APSB v1 bank. Both Web and device build orchestration call that same compiler and materialize its exact bytes as `sounds.bank`. The current Host runtimes still use the legacy PCM paths; consuming the bank is a later runtime cutover, not a second resource model.
