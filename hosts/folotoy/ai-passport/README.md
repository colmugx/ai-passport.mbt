...
- Display: ST7789P3 240x320 SPI panel. The Host reports its full 240x320 RGB565 drawing size and optional backlight. `display_bridge` sends rows unscaled in double-buffered DMA strips of 240x20 physical pixels (9,600 bytes each), two buffers outstanding, sixteen strips per frame, with per-strip completion semaphores and byte-swapped RGB565 at this one device boundary. There is no second full physical framebuffer. The 40 MHz SPI transfer lower bound for a 153,600-byte physical frame is about 30.7 ms (~32.5 FPS) before overhead.
