...
`passport build --host folotoy-ai-passport` materializes a build workspace at `<project>/.passport/folotoy-ai-passport/`: it copies these host files there, writes the contract's `pcmLoop` asset (when one is declared; a no-audio application embeds no music) as workspace-root `passport_music.pcm`, captures the project device entry's MoonBit C into `components/moonbit_device/generated/moonbit/`, connects the external FoloToy checkout (below), then runs `idf.py reconfigure` and `idf.py build` inside the workspace. `flash.sh` and `monitor.sh` are thin `idf.py` wrappers (pass `-p PORT` through) meant to run there.
