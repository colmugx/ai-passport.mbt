...
- Audio: ES8311 codec (address 0x18) on the same I2C bus. `sound_player` embeds the shared APSB bank in flash without copying PCM payloads to heap. Four independent playback slots feed one task that sums 240-sample chunks, clamps to PCM16, and uniquely owns codec writes. `microphone_bridge` reads I2S RX independently into a bounded 4096-sample queue; queue loss and failures are observable. Pause/resume/stop and positions are per playback; master volume/mute is published atomically. An empty bank is valid and keeps every `play` request explicitly unavailable. Input and output use signed PCM16 little-endian, mono, 16000 Hz.
