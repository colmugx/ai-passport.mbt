...
`passport build --host folotoy-ai-passport` materializes a build workspace at `<project>/.passport/folotoy-ai-passport/`. Before copying the current Host, the CLI validates all declared sounds and compiles the shared APSB v1 bytes. It then removes stale Host/source/generated entries while preserving only the incremental ESP-IDF state (`build/`, `managed_components/`, `sdkconfig`, `sdkconfig.old`), so files removed or renamed by a newer SDK cannot survive into firmware, and writes `sounds.bank` at the workspace root. The current firmware does not consume that bank until the sound-runtime cutover; its transitional path still writes the contract's `pcmLoop` asset (when one is declared; a no-audio application embeds no legacy music) as `passport_music.pcm`. The build then captures the project device entry's MoonBit C into `components/moonbit_device/generated/moonbit/`, connects the external FoloToy checkout (below), and runs `idf.py reconfigure` plus `idf.py build`. `flash.sh` and `monitor.sh` are thin `idf.py` wrappers (pass `-p PORT` through) meant to run there.
