...
- Audio: ES8311 codec (address 0x18) on the same I2C bus. `music_stream` embeds the canonical PCM asset into flash via ELF embedding (no heap copy) and streams it in 240-sample chunks with atomic volume/mute publication and loop wrap at end of stream. The format is fixed: signed PCM16 little-endian, mono, 16000 Hz.
