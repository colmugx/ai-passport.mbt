...
- Audio: ES8311 codec (address 0x18) on the same I2C bus. `music_stream` embeds the project's PCM asset into flash via ELF embedding (no heap copy) when the project declares one, and streams it in 240-sample chunks with atomic volume/mute publication and loop wrap at end of stream; with no declared asset the audio output still comes up but playback reports unavailable. The format is fixed: signed PCM16 little-endian, mono, 16000 Hz.
...
`passport build --host folotoy-ai-passport` materializes a build workspace at `<project>/.passport/folotoy-ai-passport/`. Before copying the current Host, the CLI removes stale Host/source/generated entries while preserving only the incremental ESP-IDF state (`build/`, `managed_components/`, `sdkconfig`, `sdkconfig.old`), so files removed or renamed by a newer SDK cannot survive into firmware. It then writes the contract's `pcmLoop` asset (when one is declared; a no-audio application embeds no music) as workspace-root `passport_music.pcm`, captures the project device entry's MoonBit C into `components/moonbit_device/generated/moonbit/`, connects the external FoloToy checkout (below), and runs `idf.py reconfigure` plus `idf.py build`. `flash.sh` and `monitor.sh` are thin `idf.py` wrappers (pass `-p PORT` through) meant to run there.
...
`partitions.csv` sizes the factory app at 0x380000 at offset 0x10000 so the flash-resident music PCM (a 76 s track is ~2.4 MB) fits next to the ~650 KB program image inside the 8 MB flash, leaving ~4.4 MB unused; sizes stay 64 KB aligned. `sdkconfig.defaults` selects the custom table and pins `CONFIG_FREERTOS_HZ=1000`. The build contract keeps the PCM at or under 0x280000 bytes; the ESP-IDF link step remains the final capacity check.
